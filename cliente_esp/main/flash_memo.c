...
int32_t le_valor_nvs(char *value_name) {
  ...
  if (res_nvs == ESP_ERR_NVS_NOT_FOUND) {
    ...
    esp_err_t res = nvs_get_i32(particao_padrao_handle, value_name, &valor);
...
void grava_valor_nvs(char *value_name, int32_t valor) {
  ESP_ERROR_CHECK(nvs_flash_init());
  // ESP_ERROR_CHECK(nvs_flash_init_partition("DadosNVS"));
  ...
  esp_err_t res = nvs_set_i32(particao_padrao_handle, value_name, valor);
  if (res != ESP_OK) {
